...
void drawMotorStatus() {
  ...
  tft.print("Motor: ");
  tft.fillRect(80, 111, 160, 16, TFT_NAVY);    // clear footer band
  tft.setFont();
  tft.setTextSize(2);
...
void setup() {
  ...
  // Set initial mode from physical button (optional)
...
BLYNK_CONNECTED() {
  ...
  Blynk.virtualWrite(V2, state);
  Blynk.virtualWrite(V3, autoMode ? 1 : 0);  // sync mode button
}
    ...
    if (motorReading != lastStableMotorBtn) {
      ...
      if (lastStableMotorBtn == LOW) {       // pressed
        if (autoMode) {
          ...
          userOffOverride = false;
          driveRelayAndSync();   // also updates V2 + display
        }
      }
    }
  }
