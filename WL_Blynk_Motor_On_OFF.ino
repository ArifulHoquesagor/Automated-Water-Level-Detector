...
// NEW: Motor push button
#define MOTOR_BTN_PIN 33        // physical ON/OFF button for motor/LED
...
void applyLogic() {
  if (autoMode) {
    ...
    // Manual: motorStatus is controlled only by V2 or the physical motor button
  }
  driveRelayAndSync();
}
...
void setup() {
  ...
  // Set button modes BEFORE reading them
  pinMode(MODE_BTN_PIN, INPUT_PULLUP);   // button to GND
  pinMode(MOTOR_BTN_PIN, INPUT_PULLUP);  // button to GND
  ...
  // Initialize Mode indicator from physical button (optional)
  ...
  applyLogic(); // reflect initial state to app/relay
}
...
void loop() {
  ...
  // --- NEW: Motor ON/OFF button (GPIO 33) ---
  int motorReading = digitalRead(MOTOR_BTN_PIN);
  if (motorReading != lastReadingMotorBtn) {
  ...
  if ((millis() - lastMotorDebounceTime) > debounceDelay) {
    if (motorReading != lastStableMotorBtn) {
      ...
      if (lastStableMotorBtn == LOW) {       // pressed
        if (autoMode) {
          if (level == 100) {
            // toggle override: if currently ON -> force OFF; if OFF -> clear override
            userOffOverride = motorStatus ? true : false;
            applyLogic();
          } else {
            // Not full in AUTO → stays OFF by rule
          ...
          userOffOverride = false;
          driveRelayAndSync();   // also updates V2
        }
      }
    }
  }
...
void drawModeIndicator(char currentMode) {
  ...
  tft.setTextSize(1);
  tft.fillRect(65, 18, 30, 20, TFT_NAVY);  // clear previous
