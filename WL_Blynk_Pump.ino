...
// This mirrors the Blynk motor switch and actual relay state
int state = 0;                 // 0 = off, 1 = on
bool motorStatus = false;      // same as state but boolean
bool userOffOverride = false;  // true when user turns V2 OFF while level==100
...
void applyRelay(int waterLevel) {
  // Auto rule + manual OFF override:
  // - If level != 100 → force OFF and clear override
  // - If level == 100:
  //     * if userOffOverride == true → keep OFF
  //     * else → ON
  if (waterLevel != 100) {
    motorStatus = false;
    userOffOverride = false;   // reset override when not full
  } else {
    motorStatus = !userOffOverride;
  }
delay(1000);
  // Drive relay (flip logic if your relay is active-LOW)
  digitalWrite(RELAY_PIN, motorStatus ? HIGH : LOW);

  // Reflect to app switch and local state
...
BLYNK_CONNECTED() {
  // Pull last app switch so we respect user's last manual choice
...
// ---------- display functions (unchanged from your code) ----------
...
void drawLevelBar(int level) {
  ...
  for (int i = 0; i < segments; i++) {
    ...
    if (i < blueSegments) tft.fillRect(x, y, segmentWidth, segmentHeight, TFT_BLUE);
    else                  tft.fillRect(x, y, segmentWidth, segmentHeight, TFT_ASH);
  }
}
...
void parseLoRaData() {
  ...
  // Apply the logic (auto + manual override)
  applyRelay(level);
}
...
BLYNK_WRITE(V2) {
  ...
  if (level == 100) {
    // While full, user can override ON → OFF or ON again
    userOffOverride = (btn == 0);
  } else {
    // Not full → always OFF, clear override
