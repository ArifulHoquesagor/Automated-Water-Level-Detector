...
// Short feedback beep (on button press)
...
// Auto/Manual logic (unchanged from your last working behavior)
void applyLogic() {
  if (autoMode) {
    // Your current rules (you may adjust separately if you changed motor policy)
    if (level != 100) {
      motorStatus = false;
...
void updateBuzzer() {
  ...
  // Priority: short click beep on button press
  if (clickBeepActive) {
    if (now >= clickBeepEnd) {
      clickBeepActive = false;
      digitalWrite(BUZZER_PIN, LOW);
      // restart alarm pattern cleanly afterwards
      buzzerActive = false;
      buzzerOnPhase = false;
      buzzerNextEvent = now;
    }
    return; // while click is active, we override alarm pattern
  }
...
void rearmBuzzerIfNeeded() {
  if (prevLevel == 100 && level != 100) {
    ...
    Blynk.virtualWrite(V4, 1);         // show enabled in app (optional)
  }
}
...
BLYNK_CONNECTED() {
  ...
  Blynk.virtualWrite(V4, buzzerMuted ? 0 : 1); // optional: show buzzer enable
}
...
void loop() {
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) {
    if (reading != lastStableBtn) {
      ...
      if (lastStableBtn == LOW) {             // pressed
        ...
        Blynk.virtualWrite(V3, autoMode ? 1 : 0);
        drawModeIndicator(currentMode);
        requestClickBeep();                    // << feedback beep
      }
    }
  }
  ...
  // continuous buzzer handling (alarm or click)
  updateBuzzer();
